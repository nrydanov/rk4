...
protected:
    ...
    virtual void derivs(double x, const std::vector<double> &state, std::vector<double> &dydx) = 0;
};
...
public:
    VanDerPolSolver(double t0, const std::vector<double> &y0, double lambda, double omega);
protected:
    void derivs(double t, const std::vector<double> &state, std::vector<double> &dydx) override;
private:
    double lambda;
    double omega2;
};
